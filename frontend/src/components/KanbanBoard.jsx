...
import { useGetStudentAssignmentQuery, useUpdateAssignmentStatusMutation } from '../services/taskAPI';
...
function KanbanBoard({ classId }) {
  ...
  const activeClassId = classId || localAssignments[0]?.classId;
  const classAssignments = localAssignments.filter(t => t.classId === activeClassId) || [];
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      await updateStatus({ id: draggableId, status: destination.droppableId }).unwrap();
  ...
  const handleFinalSubmit = async () => {
    if (!submissionLink.trim()) return toast.error("Submission link is required!");
    const updatedTasks = localAssignments.map(task => 
      task._id === pendingTaskId ? { ...task, status: "Submitted" } : task
    );
    setLocalAssignments(updatedTasks);

    try {
      await updateStatus({ id: pendingTaskId, status: "Submitted", submissionLink }).unwrap();
    ...
    <div className="h-[calc(100vh-100px)] flex flex-col overflow-hidden px-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-black text-slate-800 tracking-tight">Assignment Tracker</h2>
        </div>
        {gradeMessage && (
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex justify-end">
          <div className="w-full max-w-md bg-white h-screen shadow-2xl p-10 flex flex-col animate-in slide-in-from-right duration-500">
            <div className="flex justify-between items-center mb-8">
               <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Submit Work</h2>
               <button onClick={() => setShowModal(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={24}/></button>
            </div>
            <p className="text-slate-500 mb-6 text-sm">Please provide the public link to your completed assignment for review.</p>
            <div className="space-y-4">
              ...
              />
              <button onClick={handleFinalSubmit} className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-3xl font-black flex items-center justify-center gap-3 shadow-xl shadow-indigo-200 active:scale-95 transition-transform">
